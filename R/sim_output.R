...
# Read seed list from file
output <- fread("calibrated_seeds.csv")
...
for (i in output[1]) {
    sim_iter_R_output(i, n, rp_mn_list, rp_pois_list,
                        partner_age_dist, exposure_age_dist, 
                        weights, splines, calibration_targets,
                        immune_scale, clearance_scale,write_smoking)
}
